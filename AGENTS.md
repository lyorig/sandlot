...
- Every SDL function/enum/struct wrapper shall have `#[doc(alias = "SDL_...")]`.
- Functions wrapping setting/getting an SDL struct property shall have `#[doc(alias = "SDL_PROP_...")]`.
- Each wrapper module corresponding to an SDL category (i.e. `src/init.rs` corresponds to [CategoryInit](https://wiki.libsdl.org/SDL3/CategoryInit/raw)) contains a commented `[x]` or `[ ]` checklist containing the functions in that category. If a module cannot map 1:1 to a SDL wiki category (for example, `src/window.rs` is a subset of [CategoryVideo](https://wiki.libsdl.org/SDL3/CategoryVideo/raw)), then only relevant functions go in the module checklist.
...
While SDL works with raw pointers, Sandlot distinguishes between handles, owned objects and references.
The types in parentheses below illustrate the names generated by the `crate::resource::resource_new!` macro (see below) for an arbitrary type `Foo`.
...
- Types generated by the `resource_new!` family of macros provide an associated `from_raw` function. It manages checking whether the passed-in pointer is null. If so, `Err(Error::current())` is returned.
