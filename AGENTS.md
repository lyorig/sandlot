...
While SDL works with raw pointers, Sandlot distinguishes between owned objects and references.
The latter is implemented using a custom type, and it's all just a pointer under the hood, so no double indirection!
