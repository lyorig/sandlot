//! Loading, and rendering with, TrueType fonts.
//!
...
use crate::{Result, error::Error, init, util::mod_reexport};
...
/// call to [`Context::init`], without a paired deinitialization request.
///
...
impl Context {
    ...
    pub fn as_ref(&self) -> init::Ref<'_, Self> {
        unsafe { init::Ref::from_handle(self.handle) }
    }
}
